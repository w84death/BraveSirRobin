	...
	*/
	...
	generateProceduralMap: function(params){
		...
		insertRoom = function(params){
			//console.log('insertRoom', params.x, params.y, params.width, params.height);
			...
			for (var y = 0; y < blueprint.height; y++) {
				for (var x = 0; x < blueprint.width; x++) {
					if(collisionData[y][x] !== 0){
						// straight walls
						if(mapData[y+1] !== undefined && mapData[y+1][x] == blueprint.tiles.floor) mapData[y][x] = blueprint.tiles.walls.top[0]; else
						if(mapData[y-1] !== undefined && mapData[y-1][x] == blueprint.tiles.floor) mapData[y][x] = blueprint.tiles.walls.bottom[0]; else
						if(mapData[y][x+1] !== undefined && mapData[y][x+1] == blueprint.tiles.floor) mapData[y][x] = blueprint.tiles.walls.left[0]; else
						if(mapData[y][x-1] !== undefined && mapData[y][x-1] == blueprint.tiles.floor) mapData[y][x] = blueprint.tiles.walls.right[0];

						// inside corners
						if(mapData[y+1] !== undefined && mapData[y][x+1] !== undefined &&
							mapData[y+1][x] == blueprint.tiles.floor && mapData[y][x+1] == blueprint.tiles.floor) mapData[y][x] = blueprint.tiles.walls.insideCorners[0];
						if(mapData[y+1] !== undefined && mapData[y][x-1] !== undefined &&
							mapData[y+1][x] == blueprint.tiles.floor && mapData[y][x-1] == blueprint.tiles.floor) mapData[y][x] = blueprint.tiles.walls.insideCorners[1];
							if(mapData[y-1] !== undefined && mapData[y][x+1] !== undefined &&
							mapData[y-1][x] == blueprint.tiles.floor && mapData[y][x+1] == blueprint.tiles.floor) mapData[y][x] = blueprint.tiles.walls.insideCorners[2];
						if(mapData[y-1] !== undefined && mapData[y][x-1] !== undefined &&
							mapData[y-1][x] == blueprint.tiles.floor && mapData[y][x-1] == blueprint.tiles.floor) mapData[y][x] = blueprint.tiles.walls.insideCorners[3];
					}else{
						if(mapData[y][x] == 0){
        ...
        // GENERATE SMALLER ROOMS
	        ...
	        // top - right
				if(main.x + main.w + 2 < blueprint.width && main.y > 1){
			        ...
			        if(x + w >= blueprint.width) x = blueprint.width - w - 1;
			        if(y + h >= blueprint.height) y = blueprint.height - w - 1;
					if(w < rooms.small) w = rooms.small;
	    			if(h < rooms.small) h = rooms.small;
		...
		this.proceduralMap = {
		    entities: [
		        {type: "EntityGrail", x: 32, y: 32, settings: {}},
		    ],

		    layer: [
		        {
		            name: "background1",
		            tilesetName: blueprint.spriteSheet,
		            repeat: false,
		            distance: 1,
...
GameLoader = ig.Loader.extend({
	...
	ig.main( '#canvas', MyGame, fps, w/z, h/z, z, GameLoader);
