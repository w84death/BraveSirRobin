			...
			for (var i = 0; i < 4; i++) {
				if(blueprint.gates[i]){
					var pos = blueprint.gates[i],
						type;
					if(params.spriteSheet == 'orange') type = 'EntityGate';
					if(params.spriteSheet == 'green') type = 'EntityGate_forest';
	...
	update: function() {
		...
		if(this.STATE == 'game'){
			...
			if(ig.input.pressed('secretA')){
				this.proceduralMap = this.generateProceduralMap({spriteSheet:'orange'});
			...
			if(ig.input.pressed('secretB')){
				this.proceduralMap =  this.generateProceduralMap({spriteSheet:'green'});
				this.loadNewLevel('procedural');
				this.sX = ((this.collisionMap.width * 0.5)<<0)*this.collisionMap.tilesize;
				this.sY = ((this.collisionMap.height * 0.5)<<0)*this.collisionMap.tilesize;
			}
